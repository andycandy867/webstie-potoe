...
import {timer, reveal_element, hide_element} from '/static/resources/module.js'

import {GLTFLoader} from '/static/resources/GLTFLoader.js';
import {Octree} from '/static/resources/Octree.js';
import {Capsule} from '/static/resources/Capsule.js';
import Stats from '/static/resources/stats.module.js';

// Variable Declaration
let clock, scene, loader, camera, renderer, stats, bullet_point
let worldOctree
const GRAVITY = 20;
...
function before_opening_sequence() {
	...
	setTimeout(() => {
		...
		let transition_before_opening
		document.addEventListener('keydown', transition_before_opening = (event) => {
			if (event.code == 'Enter') {
				document.removeEventListener('keydown', transition_before_opening)
				before_continue.style.display = 'none'
...
function opening_sequence() {
	...
	reveal_element(elements[1], 2000)
	reveal_element(elements[2], 2500)
	...
	reveal_element(elements[5], 22000)
	reveal_element(elements[6], 23000)
	...
	reveal_element(elements[8], 35000)
	reveal_element(elements[9], 35000)
	hide_element(elements[7], 39000)
	hide_element(elements[8], 39000)
	hide_element(elements[9], 39000)

	reveal_element(elements[10], 45000)
	reveal_element(elements[11], 49000)
	reveal_element(elements[12], 55000)
	hide_element(elements[10], 60000)
	hide_element(elements[11], 60000)
	hide_element(elements[12], 60000)

	reveal_element(elements[13], 65000)
	reveal_element(elements[14], 69000)
	hide_element(elements[13], 72000)
	hide_element(elements[14], 72000)

	reveal_element(elements[15], 76000)
	hide_element(elements[15], 78000)
...
Ammo().then(before_opening_sequence)
//Ammo().then(start)
...
function setup_graphics() {
	...
	bullet_point = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), new THREE.MeshBasicMaterial({color: 0xffff00}))
...
function load_models() {
	...
	loader.load('carrot_blaster_joined_but_anim.glb', (gltf) => {
		...
		animation = mixer.clipAction(clip, carrot_blaster_model)
...
function setup_event_handlers() {
	...
	container.addEventListener('mousedown', handle_mousedown_container);
...
function handle_mousedown_container(event) {
		...
		for (const enemy_type in wave_enemy_types[wave]) {
			if (Object.prototype.hasOwnProperty.call(wave_enemy_types[wave], enemy_type)) {
				wave_enemy_types[wave][enemy_type].forEach(async enemy_position => {
					...
					wave_enemies_gui.innerHTML = `Enemies Remaining - [${enemies.length}]`
				})
			}
		}
	}, 2500)
...
async function scale_down(mesh) {
	let scaler = 1
...
function shooting() {
	if (mouse_states[0]) {
		...
		if (carrot_blaster_alt_debounce && carrot_blaster_alt_charge < 100) {
...
function kill_if_oob() {
	...
	enemies.forEach(enemy => {
		if (enemy.mesh && enemy.mesh.position.y <= -30) {
